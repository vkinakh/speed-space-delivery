    ...
    for (let i = 0; i < mainPlanets.length; ++i) {
        ...
        for (let j = 0; j < mainPath.length; ++j) {
            if (mainPath[j].source == tempName) {
                ...
                tempArr.push({
                    to: x,
                    weight: mainPath[j].weight,
                    price: mainPath[j].price
                });
            }
    ...
    for (let i = 0; i < freeShips.length; ++i) {
        ...
        for (let j = 0; j < optimalPath.length; ++j) {
            ...
            let section = {
                price: tempPrice,
                time: tempTime,
                length: optimalPath[j][0]
            };
            sections.push(section);
        }
        let tempObj = {
            'shipID': freeShips[i].id,
            'properties': sections
        };
...
function GetMainPath(planets, path) {
    ...
    for (let i = 0; i < path.length; i++) {
        ...
        let tempPath = {
            ...
            weight: (path[i].length * path[i].difficulty),
            price: path[i].price
        };
        ...
        if (planets[IndexInArrayByName(planets, container.from)].moonOf == planets[IndexInArrayByName(planets, container.to)].moonOf) {
            ...
            for (let i = 0; i < checkASatellite.length; i++) {
                ...
                temp.push({
                    time: tempTime,
                    price: tempPrice,
                    'length': path[indexPathA].length + path[indexPathB].length,
                    shipId: checkASatellite[i].id,
                    path: Route
                });
            }
            return temp; /////////////////////////////////////////////////////
                        ...
                        for (let j = 0; j < checkEndSatellite.length; ++j) {
                            for (let z = 0; z < checkASatellite.length; ++z) {
                                ...
                                temp.push({
                                    time: tempTime,
                                    price: tempPrice,
                                    'length': minWeigth + path[indexPathA].length + path[indexPathB].length,
                                    shipId: arrShipId,
                                    path: minPath
                                });
                            }
                ...
                for (let i = 0; i < checkBaseShuttle.length; i++) {
                    for (let j = 0; j < checkASatellite.length; ++j) {
                        ...
                        temp.push({
                            time: tempTime,
                            price: tempPrice,
                            'length': minWeigth + path[indexPathA].length,
                            shipId: arrShipId,
                            path: minPath
                        });
                    }
    ...
    else if (Array.isArray(checkBSatellite)) //B is satellite for this main planet - array with free ships from base planet to satellite
    {
        ...
        for (let i = 0; i < checkBSatellite.length; i++) {
            ...
            temp.push({
                time: tempTime,
                price: tempPrice,
                'length': path[indexPathB].length,
                shipId: checkBSatellite[i].id,
                path: Route
            });
        }
        return temp; /////////////////////////////////////////////////////
            ...
            for (let i = 0; i < checkBaseShuttle.length; i++) {
                for (let j = 0; j < checkEndSatellite.length; ++j) {
                    ...
                    temp.push({
                        time: tempTime,
                        price: tempPrice,
                        'length': minWeigth + path[indexPathB].length,
                        shipId: arrShipId,
                        path: minPath
                    });
                }
