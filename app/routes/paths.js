                ...
                if(person.permission==='admin'||person.permission==='operator'){
                    ...
                    pathModel.find(query, '-_id -__v', {sort: {id: 1}}, function(err, result){
                        if (err) res.status(502).send('Error while querying paths database');
                            ...
                            res.json(result);   
                        }else res.status(502).send('Can not find any paths');
                    });
                        ...
                        planetModel.find({$or: [{name: newPath.source}, {name: newPath.target}]}, function(err, result){
                            if (err) res.status(502).send('Error while querying planet database');
                                ...
                                if(newPath.source.type==='moon'&&newPath.target.type==='planet'){
                                    ...
                                    }else res.status(502).send('Can not create path between satellite and other base planet');
                                ...
                                }else res.status(502).send('Can not create path between this points');
                            }else res.status(502).send('Given locations not found');
                        });
                    }else res.status(502).send('Please specify all path parameters');
                }else res.status(401).send('Not enough permission');
            }else res.status(401).send('User not found');
        });
    })
                ...
                if(person.permission==='admin'){
                    ...
                    if (query.id||query.source){
                        pathModel.findOne(query, function(err, result){
                            ...
                            }else res.status(502).send('Can not find given path');
                        })
                    }else res.status(502).send('Please specify id or source/target points');
                }else res.status(401).send('Not enough permission');
            }else res.status(401).send('User not found');
        });
    })
